...
This project uses a dynamic, per-user PDA model to verify whitelisting during token transfers. Here is how the verification flow works:

```mermaid
graph TD
    A["<b>1. The Blueprint</b><br/>(ExtraAccountMetaList)"] -->|Defines seeds| B["<b>2. Automatic Derivation</b><br/>(Token-2022 Program)"]
    B -->|Calculates exact PDA| C["<b>3. Anchor Validation</b><br/>(Transfer Hook Program)"]
    C -->|Check Seeds & Lamports| D{"<b>4. Result</b>"}
    
    subgraph "Step 1: The Blueprint"
    A1["Meta: [b'whitelist', SourceOwner]"]
    end

    subgraph "Step 2: Derivation"
    B1["Takes Actual Sender Address<br/>from the transfer transaction"]
    B2["findProgramAddressSync(seeds)"]
    end

    subgraph "Step 3: Validation"
    C1["seeds = [b'whitelist', owner.key()]"]
    C2["Account initialized? (lamports > 0)"]
    end
    
    D -->|Match & Exists| E["Transfer Approved"]
    D -->|Missing or Wrong| F["Transfer Blocked (Error)"]
```

### 1. The Blueprint (ExtraAccountMetaList)
The program stores an `ExtraAccountMetaList` account that acts as an instruction manual for the Token-2022 program. It defines a rule that tells Solana: *"To find the whitelist account for this transfer, derive a PDA using the string 'whitelist' and the address of the source account owner."*

### 2. Automatic Derivation
Whenever a transfer is initiated, the **Token-2022 program** reads this blueprint and automatically calculates the exact PDA for that specific user. It then "injects" this account into the transfer hook instruction.

### 3. Anchor's Built-in Validation
When the transfer hook program receives the request, Anchor's account validation logic kicks in:
- **Seed Check**: It re-derives the PDA seeds to ensure the provided account is exactly what it claims to be.
- **Initialization Check**: It verifies that the account actually exists and contains data (it was created via `add_to_whitelist`).
- **Owner Check**: It ensures the account is owned by the whitelist program.

If any of these checks fail (e.g., the user was never whitelisted), the transaction is blocked before the program logic even executes.
