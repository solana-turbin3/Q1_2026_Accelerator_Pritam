...
describe("whitelist-transfer-hook", () => {
  ...
  const getWhitelistPDA = (user: anchor.web3.PublicKey) => {
  ...
  const adminWhitelist = getWhitelistPDA(provider.publicKey);
  ...
  it("Initializes the Whitelist", async () => {
    ...
    console.log("\nWhitelist initialized:", adminWhitelist.toBase58());
    console.log("Transaction signature:", tx);
  });

  it("Add user to whitelist", async () => {
    const user = anchor.web3.Keypair.generate().publicKey;
    const userWhitelist = getWhitelistPDA(user);

    const tx = await program.methods.addToWhitelist(user)
      .accountsPartial({
        admin: provider.publicKey,
    ...
    console.log("\nUser added to whitelist:", user.toBase58());
    console.log("Transaction signature:", tx);
  });
  ...
  it("Remove user from whitelist", async () => {
    ...
    // First add the user
    await program.methods.addToWhitelist(user)
      .accountsPartial({
        admin: provider.publicKey,
        whitelist: userWhitelist,
        systemProgram: anchor.web3.SystemProgram.programId,
    ...
    // Then remove them
    const tx = await program.methods.removeFromWhitelist(user)
      .accountsPartial({
        admin: provider.publicKey,
        whitelist: userWhitelist,
      })
    ...
    console.log("\nUser removed from whitelist:", user.toBase58());
    console.log("Transaction signature:", tx);
  });
  ...
  it('Transfer Hook with Extra Account Meta', async () => {
    ...
    transferInstruction.keys.push(
      ...
      { pubkey: adminWhitelist, isSigner: false, isWritable: false },
