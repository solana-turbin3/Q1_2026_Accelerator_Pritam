...
use anchor_spl::{
    token_2022::spl_token_2022::{
        extension::{
            BaseStateWithExtensionsMut, PodStateWithExtensionsMut, transfer_hook::TransferHookAccount
        }, 
        pod::PodAccount
    }, token_interface::{
        Mint, 
...
impl<'info> TransferHook<'info> {
    ...
    pub fn transfer_hook(&mut self, _amount: u64) -> Result<()> {
        ...
        msg!("Source token owner: {}", self.owner.key());
        msg!("Destination token owner: {}", self.destination_token.owner);
        msg!("Whitelisted PDA: {}", self.whitelist.key());
